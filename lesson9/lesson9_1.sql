...
SELECT "name" AS 站名,date_part('year',"日期") AS "年份",SUM("進站人數") AS "每年進站人數"
FROM "每日各站進出站人數" LEFT JOIN "台鐵車站資訊" ON "車站代碼" = "stationCode"
WHERE "name" = '基隆'
...
*/
SELECT "stationName" AS "車站名稱", AVG("進站人數") AS "2022 年平均每日進站人數"
FROM "每日各站進出站人數" LEFT JOIN "台鐵車站資訊"  ON "車站代碼" = "stationCode"
WHERE DATE_PART('year', "日期") = 2022
GROUP BY "stationCode", "stationName"
HAVING AVG("進站人數")  > 20000
ORDER BY AVG("進站人數")  DESC;

SELECT "name" AS 站名,
       ROUND(AVG("進站人數")) AS "2022年平均每日進站人數"
FROM "每日各站進出站人數"
